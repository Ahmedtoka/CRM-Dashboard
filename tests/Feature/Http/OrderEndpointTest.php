...
use App\Models\{ChannelAccount, City, Conversation, Customer, Order, Product, ProductVariant, User};
...
it('rejects moderator discounts with 403 and empty items with 422', function () {
    ...
    $this->actingAs($mod)->postJson($url, ['type'=>'cod', 'items'=>[['variant_id'=>$this->variant->id, 'qty'=>1]], 'discount'=>50])->assertForbidden();
    $this->actingAs($mod)->postJson($url, ['type'=>'cod', 'items'=>[]])->assertStatus(422);
});
