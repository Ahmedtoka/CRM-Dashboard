...
function addFirst(): void {
    const first = results.value.find((v) => !isOutOfStock(v));
    if (first) emit('add', first);
}

// Out of stock and not allowed to oversell (Shopify `inventory_policy: deny`).
function isOutOfStock(variant: ProductVariant): boolean {
    return variant.stock <= 0 && variant.inventory_policy !== 'continue';
}
</script>
    ...
    <div>
        ...
        <ul class="scrollbar-thin mt-2 max-h-44 overflow-y-auto rounded-md border">
            ...
            <li v-for="variant in results" :key="variant.id" class="border-b last:border-b-0">
                <button
                    type="button"
                    class="flex w-full items-center gap-2 px-2.5 py-1.5 text-start hover:bg-muted disabled:cursor-not-allowed disabled:opacity-40 disabled:hover:bg-transparent"
                    :disabled="isOutOfStock(variant)"
                    @click="emit('add', variant)"
                >
                    <img v-if="variant.image_url" :src="variant.image_url" alt="" class="size-8 shrink-0 rounded object-cover" />
                    <span class="min-w-0 flex-1">
                        ...
                        <span class="block truncate text-2xs text-muted-foreground">
                            ...
                            ·
                            <span :class="{ 'font-medium text-red-600': isOutOfStock(variant) }">
                                {{ isOutOfStock(variant) ? t('order.out_of_stock') : t('order.stock', { n: variant.stock }) }}
                            </span>
                        </span>
                    </span>
