...
<template>
    <div class="grid gap-4 lg:grid-cols-2">
        <section class="rounded-lg border bg-card text-xs">
            <h2 class="flex items-center gap-1.5 border-b px-3 py-2 text-sm font-medium"><CreditCard class="size-4" aria-hidden="true" />{{ t('simulator.orders.title') }}</h2>
            ...
            <ul class="divide-y">
                <li v-for="order in orders" :key="order.id" class="flex items-center gap-2 px-3 py-2">
                    ...
                    <button type="button" class="inline-flex h-7 items-center gap-1 rounded-md border px-2.5 font-medium hover:bg-muted disabled:opacity-50" :disabled="sim.busy.value !== null" @click="pay(order)">
        ...
        <section class="rounded-lg border bg-card text-xs">
            <h2 class="flex items-center gap-1.5 border-b px-3 py-2 text-sm font-medium"><Truck class="size-4" aria-hidden="true" />{{ t('simulator.shipments.title') }}</h2>
            ...
            <ul class="divide-y">
                <li v-for="shipment in shipments" :key="shipment.id" class="flex items-center gap-2 px-3 py-2">
                    ...
                    <button type="button" class="inline-flex h-7 items-center gap-1 rounded-md border px-2.5 font-medium hover:bg-muted disabled:opacity-50" :disabled="sim.busy.value !== null" @click="advance(shipment)">
